...
var import_devalue = require("devalue");
var set_cookie_parser = __toESM(require("set-cookie-parser"));
var _use_hashes, _script_needs_csp, _style_needs_csp, _directives, _script_src, _style_src, _nonce;
function afterUpdate() {
}
function set_prerendering(value) {
}
const Root = (0, import_chunks.c)(($$result, $$props, $$bindings, slots) => {
  let { stores } = $$props;
  let { page } = $$props;
  ...
  {
    (0, import_chunks.s)("__svelte__", stores);
  }
...
${components[1] ? `${(0, import_chunks.v)(components[0] || import_chunks.m, "svelte:component").$$render($$result, { data: data_0 }, {}, {
    default: () => {
      return `${components[2] ? `${(0, import_chunks.v)(components[1] || import_chunks.m, "svelte:component").$$render($$result, { data: data_1 }, {}, {
        default: () => {
          return `${(0, import_chunks.v)(components[2] || import_chunks.m, "svelte:component").$$render($$result, { data: data_2, form }, {}, {})}`;
        }
      })}` : `${(0, import_chunks.v)(components[1] || import_chunks.m, "svelte:component").$$render($$result, { data: data_1, form }, {}, {})}`}`;
    }
  })}` : `${(0, import_chunks.v)(components[0] || import_chunks.m, "svelte:component").$$render($$result, { data: data_0, form }, {}, {})}`}
...
function data_response(data) {
  try {
    return new Response(`window.__sveltekit_data = ${(0, import_devalue.devalue)(data)}`, {
      headers: {
...
function get_option(nodes, option) {
  return nodes.reduce((value, node) => {
    ...
    return (_b2 = (_a2 = (_a = node == null ? void 0 : node.shared) == null ? void 0 : _a[option]) != null ? _a2 : (_b = node == null ? void 0 : node.server) == null ? void 0 : _b[option]) != null ? _b2 : value;
  }, void 0);
}
...
async function render_endpoint(event, mod, state) {
  ...
  const prerender = (_a = mod.prerender) != null ? _a : state.prerender_default;
...
function is_action_json_request(event) {
  var _a;
  const accept = negotiate((_a = event.request.headers.get("accept")) != null ? _a : "*/*", [
    "application/json",
    "text/html"
  ]);
...
function get_cookies(request, url) {
  var _a;
  const initial_cookies = cookie.parse((_a = request.headers.get("cookie")) != null ? _a : "");
  const new_cookies = [];
  const cookies = {
    get(name, opts) {
...
function create_fetch({ event, options, state, route, prerender_default, resolve_opts }) {
  ...
  function get_cookie_header(url, header) {
    ...
    const combined_cookies = {
      ...initial_cookies,
      ...new_cookies,
      ...cookie.parse(header != null ? header : "")
    };
...
async function load_data({ event, fetcher, node, parent, server_data_promise }) {
  ...
  if (!((_a = node == null ? void 0 : node.shared) == null ? void 0 : _a.load)) {
    return (_a2 = server_data_node == null ? void 0 : server_data_node.data) != null ? _a2 : null;
  }
  const load_event = {
    url: event.url,
    params: event.params,
...
function writable(value, start = import_chunks.n) {
  let stop;
  const subscribers = /* @__PURE__ */ new Set();
  function set(new_value) {
    if ((0, import_chunks.a)(value, new_value)) {
      value = new_value;
      if (stop) {
  ...
  function subscribe(run, invalidate = import_chunks.n) {
    const subscriber = [run, invalidate];
    subscribers.add(subscriber);
    if (subscribers.size === 1) {
      stop = start(set) || import_chunks.n;
    }
    run(value);
    return () => {
...
function serialize_data(fetched, filter, prerendering = false) {
  ...
  if (!prerendering && fetched.method === "GET" && cache_control) {
    const match = (_a = /s-maxage=(\d+)/g.exec(cache_control)) != null ? _a : /max-age=(\d+)/g.exec(cache_control);
    if (match) {
      const ttl = +match[1] - +(age != null ? age : "0");
...
async function render_response({
  ...
  let rendered;
  const form_value = (action_result == null ? void 0 : action_result.type) === "success" || (action_result == null ? void 0 : action_result.type) === "invalid" ? (_a2 = action_result.data) != null ? _a2 : null : null;
  if (page_config.ssr) {
    const props = {
      stores: {
        page: writable(null),
  ...
  try {
    serialized.data = (0, import_devalue.devalue)(branch.map(({ server_data }) => server_data));
  } catch (e) {
    const error3 = e;
  ...
  if (form_value) {
    serialized.form = (0, import_devalue.devalue)(form_value);
  }
  const init_app = `
		...
		start({
			...
			hydrate: ${page_config.ssr ? `{
				...
				params: ${(0, import_devalue.devalue)(event.params)},
...
async function respond_with_error({ event, options, state, status, error: error2, resolve_opts }) {
  ...
  try {
    ...
    const ssr = (_a = get_option([default_layout], "ssr")) != null ? _a : true;
    if (ssr) {
    ...
    return await render_response({
      ...
      page_config: {
        ssr,
        csr: (_b = get_option([default_layout], "csr")) != null ? _b : true
      },
      status,
      error: handle_error_and_jsonify(event, options, error2),
      branch,
...
async function render_page(event, route, page, options, state, resolve_opts) {
  ...
  try {
    ...
    const should_prerender = (_a = get_option(nodes, "prerender")) != null ? _a : false;
    if (should_prerender) {
    ...
    if (get_option(nodes, "ssr") === false) {
      return await render_response({
        ...
        page_config: {
          ssr: false,
          csr: (_b = get_option(nodes, "csr")) != null ? _b : true
        },
        status,
        error: null,
        event,
    ...
    if (state.prerendering && should_prerender_data) {
      const body = `window.__sveltekit_data = ${(0, import_devalue.devalue)({
        type: "data",
    ...
    return await render_response({
      ...
      resolve_opts,
      page_config: {
        csr: (_c = get_option(nodes, "csr")) != null ? _c : true,
        ssr: true
      },
      status,
      error: null,
...
async function render_data(event, route, options, state) {
  ...
  try {
    ...
    const invalidated = (_a2 = (_a = event.url.searchParams.get("__invalid")) == null ? void 0 : _a.split("").map((x) => x === "y")) != null ? _a2 : node_ids.map(() => true);
    let aborted = false;
...
class Server {
  constructor(manifest) {
    this.options = {
      ...
      handle_error: (error2, event) => {
        var _a;
        return (_a = this.options.hooks.handleError({
          error: error2,
          event,
          get request() {
            throw new Error("request in handleError has been replaced with event. See https://github.com/sveltejs/kit/pull/3384 for details");
          }
        })) != null ? _a : { message: "Internal Error" };
      },
      hooks: null,
      manifest,
      paths: { base, assets },
  ...
  async init({ env }) {
    ...
    if (!this.options.hooks) {
      const module2 = await Promise.resolve().then(() => __toESM(require("./chunks/hooks.js")));
