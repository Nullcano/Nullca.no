<script context="module">
	...
	export async function load({ params }) {
		const { slug } = params;
		return { props: { item: data.find((i) => i.slug === slug) } };
	}
</script>
...
<svelte:head>
	<title>{item.title} \ Concepts \ Steffen Wold</title>
	<meta property="og:title" content="{item.title} \ Concepts \ Steffen Wold" />
	<meta property="og:site_name" content="Steffen Wold" />
	<meta property="og:url" content="https://steffen.id/concepts/{item.slug}" />
...
<div class="button cta" on:click={() => goto("/concepts")}>
